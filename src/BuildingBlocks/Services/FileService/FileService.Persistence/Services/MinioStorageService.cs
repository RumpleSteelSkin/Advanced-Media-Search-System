    ...
    : IFileStorageService
{
    ...
    {
        await EnsureBucketExistsAsync(_settings.BucketName, cancellationToken);
        ...
        return $"{(_settings.WithSsl ? "https" : "http")}://{_settings.Endpoint}/{_settings.BucketName}/{fileName}";
    }
    ...
    public async Task<string> GetObjectDetailAsync(string objectName, CancellationToken cancellationToken = default)
    {
        var mediaObject = await _client.StatObjectAsync(
            new StatObjectArgs().WithBucket(_settings.BucketName).WithObject(objectName), cancellationToken);
